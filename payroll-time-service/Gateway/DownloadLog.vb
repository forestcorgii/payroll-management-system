...
Namespace Gateway
    Public Class DownloadLog_
        Public Shared Function Find(databaseManager As utility_service.Manager.Mysql, payrollDate As Date, payrollCode As String) As Model.DownloadLog_
            Dim downloadLoag As Model.DownloadLog_ = Nothing
            Using reader As MySqlDataReader = databaseManager.ExecuteDataReader(String.Format("SELECT * FROM payroll_management.download_log WHERE payroll_date='{0}' AND payroll_code='{1}' ORDER BY log_created DESC LIMIT 1;", payrollDate.ToString("yyyy-MM-dd"), payrollCode))
                If reader.HasRows Then
                    reader.Read()
                    downloadLoag = New Model.DownloadLog_(reader)
                End If
            End Using
            Return downloadLoag
        End Function
        Public Shared Function Save(databaseManager As utility_service.Manager.Mysql, downloadLog As Model.DownloadLog_) As Model.DownloadLog_
        ...
        End Function
        Public Shared Sub Update(databaseManager As utility_service.Manager.Mysql, downloadLog As Model.DownloadLog_)
