    ...
    Public Class PayrollTime_
        Public Shared Sub ProcessPayrollTime(databaseManager As utility_service.Manager.Mysql, payrollDate As Date, payrollTime As Model.PayrollTime)
            Try
                ...
                'Dim _employee As Employee.Model = Employee.Gateway.Find(databaseManager, ee_id:=payrollTime.EE_Id)
                'Dim ee_id As String = 0
                'If _employee Is Nothing Then
                '    _employee = Employee.Gateway.Save(databaseManager, New Employee.Model() With {.EE_Id = payrollTime.EE_Id})
                'End If
        ...
        Public Shared Function CompleteDetail(databaseManager As utility_service.Manager.Mysql, payrollTime As Model.PayrollTime) As Model.PayrollTime
            'Try
            '    payrollTime.EE = Employee.Gateway.Find(databaseManager, payrollTime.EE_Id)
            'Catch ex As Exception
        ...
        Public Shared Sub SavePayrollTimeToDBF(databaseManager As utility_service.Manager.Mysql, payrollDate As Date, payroll_code As String, bank_category As String, dbfPath As String)
            Try
                ...
                Dim payrollTimes As List(Of Model.PayrollTime) = Gateway.PayrollTime_.Collect(databaseManager, payroll_code:=payroll_code, bank_category:=bank_category, payroll_date:=payrollDate.ToString("yyyy-MM-dd"), completeDetail:=True)
        ...
        Public Shared Sub ExportTransferLog(databaseManager As utility_service.Manager.Mysql, location As String, payrollDate As Date, payroll_code As String)
            ...
            'Dim payrollCodes As List(Of Payroll.Code.Model) = Payroll.Code.Gateway.CollectFromPreviousCutOff(databaseManager, payroll_code, previousCutoffRange)
            'For Each payrollCode As Payroll.Code.Model In payrollCodes
            '    Dim ee As Employee.Model = Employee.Gateway.Find(databaseManager, payrollCode.EE_Id)
