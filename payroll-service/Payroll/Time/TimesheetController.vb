...
Namespace Payroll
    ...
    Public Class TimesheetController
        Public Shared Function ToDBFRecordFormat(payrollTime As TimesheetModel) As String()
        ...
        End Function
        Public Shared Sub ToEERowFormat(row As IRow, payrollTime As TimesheetModel)
        ...
        Public Shared Sub ProcessPayrollTime(databaseManager As utility_service.Manager.Mysql, payrollDate As Date, payrollTime As time_module.Model.PayrollTime, loggingService As Logging.LoggingService)
                ...
                Dim allowanceLog As New AdjustmentBillingModel
                    With allowanceLog
                        ...
                        .Adjust_Type = AdjustmentChoices.ADJUST1
                    End With
                    AdjustmentBillingGateway.SaveAdjustmentLog(databaseManager, allowanceLog)
        ...
        Public Shared Function CompleteDetail(databaseManager As utility_service.Manager.Mysql, payrollTime As TimesheetModel) As TimesheetModel
            Try
        ...
        Private Shared Function GetDBFFields() As List(Of DotNetDBF.DBFField)
            ...
            Public Shared Sub SavePayrollTimeToDBF(databaseManager As utility_service.Manager.Mysql, payrollDate As Date, payroll_code As String, bank_category As String, dbfPath As String)
                Try
                    Directory.CreateDirectory(String.Format("{0}\{1}", dbfPath, payroll_code))
        ...
        Private Shared Sub ExportDBF(location As String, payrollDate As Date, records As List(Of TimesheetModel))
        ...
        Private Shared Sub ExportEFile(location As String, payrollDate As Date, payroll_code As String, bank_category As String, records As List(Of TimesheetModel))
            If records.Count = 0 Then Exit Sub

            Dim cutoffRange As Date() = Payroll.PayrollController.GetCutoffRange(payrollDate)
        ...
        Public Shared Sub ExportTransferLog(databaseManager As utility_service.Manager.Mysql, location As String, payrollDate As Date, payroll_code As String)
                Dim transferDetails As New List(Of String())
                Dim previousPayrollDate As Date = Payroll.PayrollController.GetPreviousPayrollDate(payrollDate)
                Dim previousCutoffRange As Date() = Payroll.PayrollController.GetCutoffRange(previousPayrollDate)
                ...
                Using transferLogWriter As IO.FileStream = New FileStream(String.Format("{0}\{1}\{1}_{2}.XLS", location, payroll_code, payrollDate.ToString("yyyyMMdd")), FileMode.Create, FileAccess.Write)
