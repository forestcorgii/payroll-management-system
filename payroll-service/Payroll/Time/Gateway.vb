...
Namespace Payroll
    Namespace Time
        Public Class Gateway
            Public Shared Function Find(databaseManager As utility_service.Manager.Mysql, payroll_name As String, ee_id As String) As Model
                Dim payrollTime As Model = Nothing
            ...
            Public Shared Function Collect(databaseManager As utility_service.Manager.Mysql, Optional location As String = "", Optional payroll_code As String = "", Optional bank_category As String = "", Optional payroll_date As String = "", Optional job_title As String = "", Optional completeDetail As Boolean = False) As List(Of Model)
                Dim payrollTimes As New List(Of Model)
                Using reader As MySqlDataReader = databaseManager.ExecuteDataReader(
                String.Format("SELECT * FROM payroll_management.payroll_time_complete where total_hours>0 AND (payroll_code='{1}' AND bank_category='{2}' AND payroll_date='{3}');", location, payroll_code, bank_category, payroll_date, job_title))
                    ...
                    While reader.Read()
                        payrollTimes.Add(New Model(reader))
                    End While
                End Using

                If completeDetail Then
                    payrollTimes.ForEach(Sub(item As Model) Controller.CompleteDetail(databaseManager, item))
                End If
