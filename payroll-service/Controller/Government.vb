...
Public Class Government
    ...
    Public Shared Sub ComputeGovernment(payroll30th_GROSS_PAY As Double, payroll15th_GROSS_PAY As Double, government As Model.Government)
        Dim monthlyGross As Double = payroll30th_GROSS_PAY = payroll15th_GROSS_PAY
    ...
    Public Shared Sub SavePayroll(databaseManager As Manager.Mysql, government As Model.Government)
        Try
            Dim command As New MySqlCommand("REPLACE INTO payroll_management.government (ee_id,monthly_gross_pay,monthly_net_pay,sss_ee,sss_er,pagibig_ee,pagibig_er,phic,withholding_tax,payroll_name)VALUES(?,?,?,?,?,?,?,?,?,?)", databaseManager.Connection)
            command.Parameters.AddWithValue("p1", government.EE_Id)
            command.Parameters.AddWithValue("p2", government.Monthly_Gross_Pay)
            command.Parameters.AddWithValue("p3", government.Monthly_Net_Pay)
